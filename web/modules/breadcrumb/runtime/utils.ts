...
export const pathBreadcrumbSegments = (path: string, rootNode: string = '/'): string[] => {
...
export const titleCase = (s: string) => {
  return s.replaceAll('-', ' ').replace(/\w\S*/g, w => w.charAt(0).toUpperCase() + w.substring(1).toLowerCase())
}
